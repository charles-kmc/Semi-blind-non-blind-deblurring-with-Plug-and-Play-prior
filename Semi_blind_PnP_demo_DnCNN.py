    ...
    for c_ in [100]:
        ...
        for alpha in [1]:
            ...
            ## --- PHASE: PnP parameters setting for warm-up and SAPG
                ...
                X_denoiser = denoiser_ryu(X_wu, model, eps, r_val, rotation = rotation)
            ...
            ## --- PHASE: SAPG Here, we estimate a,b, rho, sigma U and X
                ...
                for it in range(1, max_iter):
                    if max_iter_wu > 0:
                        ...
                        r_val = random.choice(ANGLE)
                        X_denoiser = denoiser_ryu(X, model, eps, r_val, rotation = rotation)
                    else:
                        X = x
   
                    G_a, G_b, G_sigma = ( grad_a(X, a_it, b_it, sigma_it), 
                                         grad_b(X ,a_it, b_it, sigma_it), 
                                         grad_sigma(X, a_it, b_it, sigma_it)
                                         )

                    # --- Metric in the SAPG stage   
                    temp_mse, temp_psnr  = mse_function(X,x); psnr_function(X,x)
                    psnr_vec.append(temp_psnr); mse_vec.append(temp_mse)

        ...
        for alpha in [1.0]:
            ...
            ## --- PHASE: Noise and degraded image
                ...
                if fix_rho:
                    ...
                    ty = ty+ "_fix_rho/RHO" + str(rho_init.cpu().numpy())
                else: 
            ...
            ## --- PHASE: PnP parameters setting for warm-up and SAPG
                ...
                U_denoiser = denoiser_ryu(X_wu, model, eps, r_val, rotation = rotation)
            ...
            ## --- PHASE: Warm-up of the PnP algorithm
                ...
                if max_iter_wu >0:
                    ...
                    for ii in range(max_iter_wu):
                        ...
                        r_val = random.choice(ANGLE)
                        U_denoiser = denoiser_ryu(U_wu, model, eps, r_val, rotation = rotation)
                        ...
                        if np.mod(ii, int(max_iter_wu/100)) ==0:
                            print(f'\nIteration = {int(ii / int(max_iter_wu/100))}%\t MSE = {temp_mse} \t PSNR = {psnr} \t rotation {r_val}\n\n')
            ...
            ## --- PHASE: SAPG Here, we estimate a,b, rho, sigma U and X
                ...
                t_sigma = [];t_rho = []; psnr_mean_x = []; mse_mean_x = []
                psnr_x_denoiser = []; psnr_u_denoiser = []
                ...
                for it in range(1, max_iter):
                    if max_iter_wu >0:
                        ...
                        r_val = random.choice(ANGLE)
                        U_denoiser = denoiser_ryu(U, model, eps, r_val, rotation = rotation)
                    else:
                        X = x
                        U = x
                ...
                if plots:
                    ...
                    if max_iter_wu>0:
                        ...
                        # --- PSNR x denoiser
                        plot1 = plt.figure()
                        plt.plot(torch.stack(psnr_x_denoiser).cpu().numpy(), label = '-psnr x denoiser-')
                        plt.legend()
                        
                        # --- PSNR u denoiser
                        plot1 = plt.figure()
                        plt.plot(torch.stack(psnr_u_denoiser).cpu().numpy(), label = '-psnr u denoiser-')
                        plt.legend()
